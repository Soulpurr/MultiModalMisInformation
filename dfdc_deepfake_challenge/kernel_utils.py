            ...
            if n > 0:
                ...
                with torch.no_grad():
                    ...
                    for model in models:
                        ...
                        preds.append(bpred)  # Collect the raw probabilities for all frames

                    frame_scores = np.mean(np.array(preds), axis=0)  # Mean of predictions across models
                    frame_images = [x[i].permute(1, 2, 0).cpu().numpy() * 255 for i in range(n)]
                    frame_images = [img.astype(np.uint8) for img in frame_images]

                    # Return probabilities for all frames
                    return frame_scores.tolist(), list(zip(frame_scores, frame_images))
    except Exception as e:
        print("Prediction error on video %s: %s" % (video_path, str(e)))

    return [0.5] * batch_size, []  # Return a default value for error cases




def predict_on_video_set(face_extractor, videos, input_size, num_workers, test_dir, frames_per_video, models,
                         strategy=np.mean, apply_compression=False):
    def process_file(i):
        filename = videos[i]
        video_path = os.path.join(test_dir, filename)
        video_score, frame_data = predict_on_video(
        ...
        return filename, video_score, frame_data

    predictions = {}
    frames_info = {}

    with ThreadPoolExecutor(max_workers=num_workers) as ex:
        for filename, score, frame_data in ex.map(process_file, range(len(videos))):
            predictions[filename] = score
            frames_info[filename] = frame_data

    return predictions, frames_info
